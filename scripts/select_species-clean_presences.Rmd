...
needed <- gsub('_', ' ', setdiff(
  gsub(' ', '_', unique(specimens$scntfcnm)), unique(tables$scntfcnm)))
...
# write out and review these again based on the density curves
...
no_cores <- parallel::detectCores()
names(splicies)
lapply(splicies, ince_writer, bs = 20) 
```
...
p <- '../results/initation_cessation_tables'
tables <- lapply(file.path(p, list.files(path = p)), repntfcnm)

f <- gsub('.shp', '', list.files('../results/PresAbs', pattern = 'shp$'))
tables <- filter(tables, ! scntfcnm %in% f)

specimens <- st_read('../data/processed/training/presences.shp', quiet = TRUE ) %>% 
  bind_rows()
  #filter(scntfcnm %in% gsub('_', ' ', names(tables))) #%>% 
  #filter(scntfcnm %in% gsub('_', ' ', unique(tables$scntfcnm)))
pca1 <- terra::rast('../results/spatial/gddPCA.tif')
specimens <- terra::extract(
  terra::rast('../results/spatial/gddPCA.tif'), specimens, bind = TRUE) |>
  st_as_sf() %>% 
...
setdiff(gsub(' ', '_', unique(specimens$scntfcnm)), unique(tables$scntfcnm))
