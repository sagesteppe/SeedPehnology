...
splicies <- Filter(function(x) nrow(x) < 51, splicies)
# splicies <- Filter(function(x) nrow(x) <= 300, splicies)
obs <- lapply(splicies[1], ince_writer)  # 54 gets a try catch error.  68 merge fails. # 71 runs forever


tibble(obd1)


# fewer than 50 records may mean no model via phenesse

ll <- pheno_abs(clusts, ince)

ggplot(ll) +
  geom_segment(aes(x = cessAbs, xend = initAbs,
                   y = PC1, yend = PC1)) +
  geom_point(aes(x = initAbs, y = PC1), size = 2, color = 'green') +
  geom_point(aes(x = cessAbs, y = PC1), size = 2, color = 'red') + 
  geom_point(aes(x = doy, y = PC1, color = ClusterID), size = 3) 
